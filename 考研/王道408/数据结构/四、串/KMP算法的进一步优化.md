...
- [[求next数组]]：nextval 的基础，必须先会求 next
- [[KMP算法]]：nextval 只是换掉 next 数组，匹配流程不变
- [[朴素模式匹配算法]]：对比整个优化链条的起点
- [[串的存储结构]]：下标从 1 的约定贯穿始终
...
- [[KMP算法]]：nextval 替换 next 的匹配算法
- [[求next数组]]：先求 next，再据规则推 nextval
- [[朴素模式匹配算法]]：$O(nm) \to O(n+m)$ 的优化链条终点
- [[串的存储结构]]：存储约定与下标规则